 ...
 * never a second copy. The registry moved into this repository when the
 * site-shell package went machinery-only (0.2.0, TODO.public track 02):
 * it was the package's src/data/components.ts, adopted verbatim and
 * reshaped to www's own data-file idiom. The shell's reference copy
 * lives at presets/www/components.mjs in the shell repository; the two
 * carry the same values by adoption, and this file is www's home copy.
 *
 * Scope split (2026-08):
...
export interface SmartComponent {
  /** Display name. */
  name: string
  /** The canonical route (served by the component's own deployment). */
  href: string
...
/** Filter helpers — the dropdown and the index sections read these. */
