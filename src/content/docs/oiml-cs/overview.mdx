---
title: 'The OIML-CS Scheme'
shortTitle: 'Overview'
description: 'The certification system as a reference package: B 18:2025, the CASCO delegation, the runtimes, the coverage machinery.'
eyebrow: 'OIML-CS · Volume overview'
order: 0
---
...
1. [The scheme architecture](/docs/oiml-cs/01-scheme-architecture) — the B 18:2025
...
2. [The CASCO foundation](/docs/oiml-cs/02-casco-foundation) — why the scheme is
...
3. [The documents corpus](/docs/oiml-cs/03-documents-corpus) — the per-document
   module convention and what each of PD-01…PD-09, CID-01, OD-01/02
   contributes.
4. [The certification workflow](/docs/oiml-cs/04-certification-workflow) — PD-05 as
   abstract processes + 34 provisions, `realized_by` binding to the
   concrete Core processes, and the `.prm` coverage discipline.
5. [The participant runtime](/docs/oiml-cs/05-participant-runtime) — the registry,
   the approval pipeline with the MC 80 % vote, and the issuance gate
   that fails closed.
6. [The operations runtime](/docs/oiml-cs/06-operations-runtime) — Scheme A MTL/ANR
   ...
   source.
7. [The coverage machinery](/docs/oiml-cs/07-coverage-machinery) — the `.prm` maps,
   the unified per-document report, the named-gap doctrine, and the
   mutation proofs that keep the gate a gate.
8. [The flow, executable](/docs/oiml-cs/08-the-flow-executable) — the certification
