---
title: 'The OIML Core Metamodel'
shortTitle: 'Overview'
description: 'The metamodel of legal metrology — the 4-layer architecture, the six modules, the dependency law.'
eyebrow: 'OIML Core · Volume overview'
order: 0
---
...
14](https://www.primmel.org/primmel-smart-docs/primmel/14-live-twins/), on the Primmel SMART docs site, ● — the twin console, the monitor
...
0. *This README* — volume overview; the 4-layer architecture; the six
   modules.
1. [Measurement vocabulary](/docs/oiml-core/01-measurement-vocabulary) — Module A:
   QuantityKind, Unit, QuantityValue, uncertainty, Measurand,
   InfluenceQuantity, Conditions, MeasurementResult, TraceabilityChain.
2. [The subject chain](/docs/oiml-core/02-subject-chain) — Module C core: Family →
   Group → Model → Sample; VIML anchoring; classification; scope and
   origin; delegation.
3. [Instrument aspects](/docs/oiml-core/03-instrument-aspects) — Module C aspects:
   attributes/parameters, capabilities, behaviors, condition sets,
   formulas, constraints — the IS/HAS/DOES instrument catalog.
4. [Identity and provenance](/docs/oiml-core/04-identity-and-provenance) — Module B:
   Manufacturer, SoftwareComponent, Marking, Sealing, CalibrationRecord,
   Certificate.
5. [Specification](/docs/oiml-core/05-specification) — Module D1: Recommendation,
   Requirement, ConformanceTest, TestMethod, TestStep.
6. [Test execution](/docs/oiml-core/06-test-execution) — Module D2: TestRun,
   EvidenceRecord, admissibility, ConstraintCheck, TestReport.
7. [Evaluation](/docs/oiml-core/07-evaluation) — Module D3: SampleEvaluation, Verdict,
   TypeEvaluation, EvaluationReport, TypeApprovalDecision.
8. [Parties and workflow](/docs/oiml-core/08-parties-and-workflow) — parties, roles,
   the certification workflow entities, lifecycle state machines.
9. [Invariants](/docs/oiml-core/09-invariants) — INV-1..14: the metamodel's laws, each
   with rationale, checks, and the shipped registry's crosswalk.
10. [Shared modules](/docs/oiml-core/10-shared-modules) — emc-disturbances,
...
*Next: [Chapter 1 — Measurement Vocabulary](/docs/oiml-core/01-measurement-vocabulary):
